...
def get_github_env() -> Optional[dict]:
    """Get environment with GitHub token set up. Returns None to use parent env.

    Subprocess env behavior:
    - env=None → Inherits parent's environment (default) - BEST for gh CLI
    - env={} → Empty environment (no variables)
    - env=custom_dict → Only uses specified variables

    We always return None to inherit the full parent environment because:
    1. gh CLI uses default authentication from keyring/config
    2. A minimal env breaks on Windows (missing SystemRoot, TEMP, etc.)
    3. GITHUB_PAT can be set in parent environment if needed
    """
    # Always return None to inherit full parent environment
    # This ensures gh CLI can use its default authentication and all
    # necessary system variables are available
    return None
