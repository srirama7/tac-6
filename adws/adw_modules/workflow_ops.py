...
def build_plan(
    ...
    issue_plan_template_request = AgentTemplateRequest(
        ...
        args=[str(issue.number), adw_id, strip_adw_comments(issue).model_dump_json(by_alias=True)],
        adw_id=adw_id,
        model="sonnet",
    )
...
def generate_branch_name(
    ...
    request = AgentTemplateRequest(
        ...
        args=[issue_type, adw_id, strip_adw_comments(issue).model_dump_json(by_alias=True)],
        adw_id=adw_id,
        model="sonnet",
    )
...
def create_commit(
    ...
    request = AgentTemplateRequest(
        ...
        args=[agent_name, issue_type, strip_adw_comments(issue).model_dump_json(by_alias=True)],
        adw_id=adw_id,
        model="sonnet",
    )
...
def create_pull_request(
    ...
    if not issue:
        ...
        issue_json = strip_adw_comments(issue).model_dump_json(by_alias=True)
...
def strip_adw_comments(issue: GitHubIssue) -> GitHubIssue:
    """Remove ADW workflow comments from issue to reduce prompt size.
    
    ADW comments follow the pattern: {adw_id}_{agent}: message
    These comments are generated by the workflow and clutter the issue.
    """
    import re
    # Pattern matches: 8-char hex ADW ID followed by underscore
    adw_comment_pattern = re.compile(r'^[a-f0-9]{8}_')
    
    filtered_comments = [
        comment for comment in issue.comments
        if not adw_comment_pattern.match(comment.body)
    ]
    
    # Create a copy with filtered comments
    issue_dict = issue.model_dump(by_alias=True)
    issue_dict['comments'] = [c.model_dump(by_alias=True) for c in filtered_comments]
    
    return GitHubIssue.model_validate(issue_dict)
